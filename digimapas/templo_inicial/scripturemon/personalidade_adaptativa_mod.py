...
class PersonalidadeAdaptativa:
    """Representa tracos de personalidade que se ajustam com o tempo."""
    def __init__(self, nome: str, tracos_iniciais: Optional[Dict[str, float]] = None) -> None:
        ...
        self._registrar_historico("Inicializacao", self.tracos)

    def avaliar_feedback(self, retorno_emocional: float, recompensa: float) -> None:
        """
        Ajusta os tracos de personalidade com base em retorno emocional (0-1) e recompensa.
    ...
    def ajustar_estilos_de_fala(self) -> Dict[str, str]:
        """
        Gera estilos de fala baseados nos tracos de personalidade.
    ...
    def _registrar_historico(self, evento: str, dados: Dict[str, float] | Dict[str, str]) -> None:
        """
        Registra um evento de mudanca da personalidade no arquivo markdown e no logger.
        """
        try:
            ...
            self.logger.exception("Erro ao registrar historico: %s", e)
